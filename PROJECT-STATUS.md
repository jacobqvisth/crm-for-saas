# CRM Project Status
Last updated: 2026-08-11 (**CAC/LTV page LIVE — PR #641**. New `/dashboard/cac-ltv` built from the CEO's `WrenchLane_Growth_Profitability_Model_v3.xlsx` (which arrived empty) to answer *"om ni har 100kr per reggad kund, när blir den lönsam?"*. Headline: 100 kr per registration is **not** a 100 kr CAC — at 3.4% signup-to-paid a payer costs ~2,941 kr, repaying in 3.7 months at 5.8× blended. The actionable finding is the per-tier spread: **One affords 36 kr per registration against Large's 374 kr (10×)**, and at today's blended cost One takes 50 months to repay vs Large's 1.8 — the arithmetic behind splitting campaigns per product. Live funnel/spend/cohort data from prod; churn, discount, per-vehicle data cost and FX are labelled sliders. Also documents two formula bugs in the CEO's workbook and five prod metric traps (`core_ai_total_cost` is cumulative, `mrr_amount_cents` is not SEK, `ad_conversions` is unusable, a 135-row single-day `canceled_at` backfill pile-up, activation is not a funnel gate). **Open:** the InfoPro/Motor per-lookup supplier rate exists in no synced table and sets the margin that multiplies every LTV; 5 of 6 channels in the workbook are unmeasurable until signups carry a first-touch source stamp. See `cc-session-log.md` "CAC/LTV page: unit-economics model from the CEO growth workbook".)

Prior: 2026-08-11 (**User Journey canvas page LIVE — PR #640**. New `/journey` Miro-style board: pan/zoom canvas with drag/drop images, sticky notes, labels and stage frames, all autosaving; `journey_boards`/`journey_items` tables + `journey-images` bucket. Seeded a "User Journey (current flow)" board mapping the real funnel landing → signup → verify → onboarding → AI-diagnostic → upgrade → Stripe checkout (40 items, 5 stages) from a live walkthrough on a fresh `jacob+5` account, with friction points flagged. See `cc-session-log.md` "User Journey canvas page + seeded current-flow board".)

Prior: 2026-06-24 (**Sequence "Finish in-progress only" toggle LIVE — PR #421**. Per-sequence `pause_new_contacts` setting: keeps sending follow-ups to already-started contacts but holds first emails for new contacts; toggle in sequence Settings + amber header badge + cron enforcement + `/api/sequences/[id]/pause-new-contacts`. Applied to the 6 cold-outreach sequences (Sverige/UK/Czech/LT/EE/LV); ~1,633 first-emails held, follow-ups still flowing. Also fixed: settings Save was wiping `allow_customers`. Diagnosed the ~15/hr send-throughput issue = head-of-line clog (oldest-first 100-row cron window, 1 email/account/run; 390 month-old Sverige rows on slow .co accounts). Plus two MillionVerifier-validated non-Swedish app-user check-in sequences (diagnosed vs no-diagnosis, 433 enrolled). See `cc-session-log.md` "Non-Swedish user check-in sequences + finish in-progress only". Prior: In-CRM Calling pipeline LIVE PRs #410–#413.)
