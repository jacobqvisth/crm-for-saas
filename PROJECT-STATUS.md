...
| Compose modal: per-email send language | "Send in" language selector on the one-off contact **Email** modal (`compose-email-modal.tsx`), defaulting to the contact's stored `language` (else English), overridable per email. Mirrors the inbox reply flow: rep composes/edits in **English**; when target ≠ English the email auto-translates at send. New `translateOutboundEmail()` in `src/lib/inbox/translate-outbound.ts` preserves HTML tags + `{{merge}}` tokens so variable-resolution/tracking run unchanged. Side-by-side "Recipient sees (Language)" preview via new `/api/ai/translate-email` (debounced); `send-email` route re-translates fresh server-side (stale preview never ships) and logs `body_en`/`sent_language` in `activities.metadata`. Language labels extracted to client-safe `src/lib/i18n/languages.ts`. Build note: OOMs (exit 137) under Codex.app node — use Homebrew node. | ✅ Merged + LIVE | #471 |
