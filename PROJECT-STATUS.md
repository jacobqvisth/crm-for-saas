# CRM Project Status
Last updated: 2026-05-20 (Inbox UX overhaul end-to-end: hide-OOO/sender filters, inbound translation + backfill, English-first viewer, auto-suggested draft, outbound translation at send time, check-replies cron timeout fix, canonical Wrenchlane knowledge file + editable `/settings/ai-knowledge`. See `cc-session-log.md` for PRs #239 → #267.)
...
| **Next** | Options: (A) Begin Sweden cold-outreach — **5,897 core-ICP shops with 3,355 sendable emails** ready for sequences (filter: `shop_type IN ('auto_repair','tire_combo','auto_glass','auto_body') AND email_status IN ('valid','catch_all') AND crm_company_id IS NULL`); (B) check-replies architectural rewrite to per-sender `messages.list` (queued from PR #254 follow-up); (C) Pre-existing `REMOVE_REASONS` route-export error from PR #150 breaks local `next build` on main — Vercel tolerates it but CI is red, worth a dedicated fix PR; (D) Phase 26 A/B testing; (E) Norway / Denmark scrape (apply same `--runs-file=` pattern); (F) Pattern-MV recipe on CZ/SK/RS rows with website-but-no-email; (G) Optional: AI classification on the residual 1,064 'other' rows if outreach performance suggests we're missing ICP | 🔜 | — |
