...
| **Next** | Options: (A) Begin Sweden cold-outreach — **5,897 core-ICP shops with 3,355 sendable emails** ready for sequences (filter: `shop_type IN ('auto_repair','tire_combo','auto_glass','auto_body') AND email_status IN ('valid','catch_all') AND crm_company_id IS NULL`); (B) CI red-on-main fix; (C) Phase 26 A/B testing; (D) Norway / Denmark scrape (apply same `--runs-file=` pattern); (E) Pattern-MV recipe on CZ/SK/RS rows with website-but-no-email; (F) Optional: AI classification on the residual 1,064 'other' rows if outreach performance suggests we're missing ICP | 🔜 | — |
