...
export const DASHBOARD_SECTIONS: DashboardSectionConfig[] = [
  ...
  },
  {
    ...
    title: "Promo Users",
    description:
      "Does giving people promo codes actually work? Seven tabs over every user ever given a coupon. Overview: who got what, how much was given up per currency, and the engagement and outreach splits. Does it work: the promo cohort against TWO comparisons — paid-no-promo (the honest one, same sales motion without a discount) and free-no-promo — on activation, repeat use, intensity per active day and payment, plus a before/after read anchored on each user's own promo date, and per-code effectiveness. Users: every discounted app user with diagnoses either side of their promo, chats, features, logins, active days, calls, emails, opens, clicks and replies. Timeline: one user's whole history in a single stream — signup, promo landing, every call with the rep who placed it, every email with the mailbox that sent it, replies, and every diagnosis with the car and what they typed. Outreach: the full call log (rep, connected, duration, outcome, days from promo) and email log (sender, sequence, opened/clicked/replied). Product use: what they searched for, cars, DTCs and symptoms. Funnel: how far each cohort gets, signup through still-active. Grains are kept apart deliberately: money per grant, behaviour per app user, outreach per contact. Association, not proof — promo recipients were not randomly chosen.",
  },
  {
