...
// GET /api/forums/thread?source=distribution&source_id=<id>
...
// comments (in priority order). Only 'distribution' is wired today.
export async function GET(request: NextRequest) {
  ...
  const grouped = await fetchAssignmentsBySource(supabase, workspaceId, "distribution", [sourceId]);
  ...
  const out = rec as unknown as DistributionRec;
