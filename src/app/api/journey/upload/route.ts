...
// Mirrors /api/email-images/upload, with a bigger cap because journey items
// are full-page screenshots.
...
async function ensureJourneyImagesBucket() {
...
export async function POST(request: NextRequest) {
  ...
  if (file.size > MAX_FILE_SIZE_BYTES) {
    return NextResponse.json({ error: "Images can be up to 10 MB." }, { status: 400 });
  }
  ...
  const { serviceSupabase, error: bucketError } = await ensureJourneyImagesBucket();
  if (bucketError) {
    return NextResponse.json({ error: bucketError }, { status: 500 });
  }

  const extension = ALLOWED_IMAGE_TYPES.get(file.type);
  const objectPath = `${workspaceId}/${Date.now()}-${randomUUID()}.${extension}`;
