...
/**
 * POST /api/companies/[id]/owner
 *
 * Set the rep ownership for a company.
 *  - { auto: true } → re-enable auto-assignment and recompute from activity
 *    (rolls up the company's own activity + all of its contacts').
...
export async function POST(
  ...
  if (parsed.auto) {
    ...
    const { error: rpcErr } = await supabase.rpc("recompute_company_owner", {
      p_company_id: id,
    });
