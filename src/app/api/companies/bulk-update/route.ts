...
const VALID_BY_FIELD: Record<string, readonly string[]> = {
  lifecycle_stage: VALID_LIFECYCLE_STAGES,
  customer_status: VALID_CUSTOMER_STATUSES,
};
...
export async function POST(request: NextRequest) {
  ...
  const { companyIds, filters, workspaceId, field, value } = body as {
    companyIds?: string[];
  ...
  const allowed = VALID_BY_FIELD[field];
  if (!allowed) return NextResponse.json({ error: "Invalid field" }, { status: 400 });
