...
export async function GET(
  ...
  const { data: session, error } = await supabase
    .from("call_sessions")
    .select(
      "id, status, direction, to_number, from_number, duration_seconds, recording_url, summary, ai_json, ai_processed_at, activity_id, contact_id, company_id, error, created_at",
    )
    .eq("id", id)
    .maybeSingle();
