// Scrape Vianor NO per-branch pages. One-pass via sitemap.
// Sitemap: https://vianor.no/sitemap.xml (filter for /verksteder/ paths)
// Pattern: /verksteder/vianor-{city-area}/
// Output: scripts/data/no-chains-vianor.json
...
const SITEMAP_URL = "https://vianor.no/sitemap.xml";
const OUT = join(DATA_DIR, "no-chains-vianor.json");
...
let urls = [...sitemapXml.matchAll(/<loc>([^<]+)<\/loc>/g)].map((m) => m[1])
  .filter((u) => /\/verksteder\/vianor-/.test(u));
...
const parseBranch = (html, url) => {
  ...
  // Fallback: Vianor renders emails as plain text, not mailto links. Regex search.
  if (!email) {
    const text = $("body").text();
    // Prefer a city-prefix @vianor.com email if present
    const vianorMatch = text.match(/[\w.-]+@vianor\.[a-z]{2,4}/i);
    if (vianorMatch) email = vianorMatch[0].toLowerCase();
    else {
      const generic = text.match(/[\w.-]+@[\w-]+\.[a-zA-Z]{2,6}/);
      if (generic) email = generic[0].toLowerCase();
    }
  }
  ...
  $("a[href^='http']").each((_, el) => {
    ...
    if (!href) return;
    if (/vianor\.|facebook\.com|instagram\.com|linkedin\.com|twitter\.com|youtube\.com|maps\.google|google\.com|wa\.me|whatsapp|tiktok\.com|pinterest\.com|spotify\.com|apple\.com|nokian/i.test(href)) return;
  ...
  // Fallback city from URL slug: /verksteder/vianor-{rest}/ — take the first city-name token
  if (!city) {
    try {
      const p = new URL(url).pathname.split("/").filter(Boolean);
      // verksteder/vianor-{city-area}
      const last = p[p.length - 1];
      if (last && last.startsWith("vianor-")) {
        const slug = last.replace(/^vianor-/, "").replace(/-partner$/, "");
        city = slug.split("-")[0];
  ...
  return {
    chain: "vianor", source_url: url, name, address, postal_code, city,
